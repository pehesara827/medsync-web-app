...
export default function DoctorProfile() {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              {PROFILE.verified && (
                <span className="flex items-center gap-1 text-[10px] font-medium bg-cyan-50 text-cyan-600 px-2 py-0.5 rounded-full">
                  <BadgeCheck className="w-3 h-3" /> Verified
                </span>
              )}
            </div>
            <p className="text-sm text-cyan-600 mt-0.5">{PROFILE.title}</p>
          </div>
        </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-6">
        <p className="font-medium text-slate-900 flex items-center gap-2 mb-4">
          <Stethoscope className="w-4 h-4 text-cyan-600" /> Contact Info
        </p>
        <div className="space-y-3 mb-5 text-sm">
          <div className="flex items-start gap-2">
            <MapPin className="w-4 h-4 text-slate-400 mt-0.5" />
            <div>
              <p className="text-slate-900">{PROFILE.contact.clinic}</p>
              <p className="text-xs text-slate-400">{PROFILE.contact.address}</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <Mail className="w-4 h-4 text-slate-400" />
            <p className="text-slate-700">{PROFILE.contact.email}</p>
          </div>
        </div>
      ...
      <div className="space-y-4">
        <div className="bg-white rounded-xl border border-slate-200 p-6">
          <p className="font-medium text-slate-900 mb-3">Clinical Expertise</p>
          <div className="flex flex-wrap gap-2">
            {PROFILE.expertise.map((e, i) => (
              <span key={i} className="text-xs font-medium bg-cyan-50 text-cyan-700 px-3 py-1.5 rounded-full">{e}</span>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-6">
          <p className="font-medium text-slate-900 flex items-center gap-2 mb-3">
            <GraduationCap className="w-4 h-4 text-cyan-600" /> Education &amp; Board
          </p>
